...
namespace PentominoesApp
{
    ...
    class Program
    {
        ...
        private static void OnSolutionFound(Solution solution)
        {
            if (SolutionIsUnique(solution))
            {
                UniqueSolutions.Add(solution);
                var board = FormatBoard(solution);
                UniqueJoinedBoards.Add(string.Join("|", board));
                DrawSolution(board);
            }
        }

        private static bool SolutionIsUnique(Solution solution)
        {
            var board1 = FormatBoard(solution);
        ...
        private static ImmutableArray<string> FormatBoard(Solution solution)
        {
